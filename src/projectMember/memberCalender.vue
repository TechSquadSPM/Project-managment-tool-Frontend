            ...
            <tr v-for="(item, index) in calendarMatrix" v-bind:key="index">
              <td v-for="(data, i) in item" v-bind:key="i" :class="{'occassion': data.occassion ? data.occassion : false}">
                <div v-for="(event,id) in monthEvents" v-bind:key="id">
                  <div v-if="data ? data.date.getDay() == eventsArray[event].day && eventsArray[event].occurrence == 'Weekly' : false">{{`${changeOccurrence(index, i)}`}} {{eventsArray[event].memo}}</div>
                  <div v-else-if="data ? data.date.getDate() == eventsArray[event].day && eventsArray[event].occurrence != 'Weekly': false">
                    {{`${changeOccurrence(index, i)}`}}{{eventsArray[event].memo}}
                  </div>
                  <div >
...
export default {
  ...
  mounted(){
    module.getEmpInprogressModule("inprogress",localStorage.getItem('empId')).then(moduledoc=>{
    for(let j=0;j<moduledoc.data.length;j++)
    {
        project.getprojectByID(moduledoc.data[j].projectId).then(projectdoc=>{
            this.projectname=projectdoc.data[0].projectName

        var year=moduledoc.data[j].moduleDeadline.slice(0,4);
        var month=moduledoc.data[j].moduleDeadline.slice(5,7);
        var day=moduledoc.data[j].moduleDeadline.slice(8,10);
        var vm = this
        vm.eventsArray.push(vm.newEvent(day,month,year,moduledoc.data[j].moduleName+"("+this.projectname));
        })
    }
    })
  },
